 ...
 */
function bindEvents(evts, el, attrs, weexEvents, appearAttached, allComponents, isNativeEvents) {
  ...
  const isCustomComponent = allComponents.indexOf(el.tag) == -1;
  /**
   ...
   */
  if (!hasBubbleParent) {
    // 自定义组件不要添加.stop,因为没有stopPropagation方法.web上执行的时候会出错!
    // 原生事件或者原生组件才添加stopPropagation方法
    if(isNativeEvents || !isCustomComponent){
      for (const k in evts) {
   ...
   * - scroll -> weex$scroll
   */
  if (evts.click && !isCustomComponent) {
  ...
  if (evts.scroll && !isCustomComponent) {
...
module.exports = function eventsHook (
  ...
  const { weexEvents, weexRegisteredComponents, aliweexComponents, weexBuiltInComponents } = this.config
  const allComponents = [...weexRegisteredComponents, ...aliweexComponents, ...weexBuiltInComponents]
  let evts = el.events
  ...
  if (evts) {
    bindEvents(evts, el, attrs, weexEvents, appearAttached, allComponents, false)
  }
  if (nativeEvts) {
    bindEvents(nativeEvts, el, attrs, weexEvents, appearAttached, allComponents, true)
  }
  else {
    delete el.nativeEvents
  }
