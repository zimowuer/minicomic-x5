...
var SUPPORT_CSS_UNIT = ['px', 'pt', 'wx', 'rpx', 'vh', 'vw', '%']
...
var SHORTHAND_LENGTH_VALIDATOR = function SHORTHAND_LENGTH_VALIDATOR(v) {
  ...
  if (!value) {
    ...
    if (reason.length > 0) {
      result.reason = function (k, v, result) {
...
var FLEX_VALIDATOR = function FLEX_VALIDATOR(v) {
  v = (v || '').toString()
  var match = v.match(LENGTH_REGEXP)
  ...
  if (['auto', 'initial', 'none'].includes(v)) {
    return { value: v }
  }
  return {
    value: null,
...
var TRANSITION_PROPERTY_VALIDATOR = function TRANSITION_PROPERTY_VALIDATOR(v) {
  ...
  if (v.split(/\s*,\s*/).every(p => !!validatorMap[p] || p === "all")) {
    return { value: v }
  }
...
//默认的校验,原样返回
var DEFAULT_VALIDATOR = function TRANSFORM_VALIDATOR(v) {
  return { value: v }
}
...
function BOX_SHADOW_VALIDATOR(v) {
  const items = v.split(/\s+/);
  if (items.length !== 4) {
    return {
      value: null,
      reason: function reason(k, v, result) {
        return 'ERROR: property value `' + v + '` is not supported for `' + util.camelCaseToHyphened(k) + '` (supported like: `box-shadow:10px 10px 10px #000000`)'
      }
    }
  }
  ...
  for (let i = 0; i < items.length; i++) {
    let res;
    if (i < 3) {
      res = LENGTH_VALIDATOR(items[i]);
    } else {
      res = COLOR_VALIDATOR([items[i]]);
    }
...
function ANIMATION_ITERATION_VALIDATOR(value) {
  if (value === "infinite") {
    return { value }
  }
...
// 支持 left/top/right/bottom/center以及百分比
function BACKGROUND_SIZE_VALIDATOR(value) {
  const reg = /^(left|top|right|bottom|center)$/;
  ...
  if (success) {
    ...
    return {
      value:null,
      reason: function reason(k, v, result) {
        return 'ERROR: property value `' + v + '` is not supported for `' + util.camelCaseToHyphened(k) + '` (supported like: `background-position:left top;support values are:`left`,`top`, `right`,`bottom`,`center`,`[number]%`)';
      }
    }
  }
}
...
var PROP_NAME_GROUPS = {
  boxModel: {
    ...
    minHeight: LENGTH_VALIDATOR,
    maxWidth: LENGTH_VALIDATOR,
    maxHeight: LENGTH_VALIDATOR,
    overflow: genEnumValidator(['visible', 'hidden']),
    ...
    boxSizing: genEnumValidator(['border-box']),
  ...
  flexbox: {
    ...
    flexWrap: genEnumValidator(['nowrap', 'wrap', 'wrap-reverse']),
    ...
    alignSelf: genEnumValidator(['stretch', 'flex-start', 'flex-end', 'center']),
    alignContent: genEnumValidator(['auto', 'center', 'flex-start', 'flex-end', 'stretch', 'space-between', 'space-around']),
    gap: LENGTH_VALIDATOR,
    rowGap: LENGTH_VALIDATOR,
    columnGap: LENGTH_VALIDATOR,
  },
  position: {
  ...
  common: {
    ...
    backgroundRepeat: genEnumValidator(['repeat', 'repeat-x', 'repeat-y', 'no-repeat']),
    backgroundSize: genEnumValidator(['auto', 'contain', 'cover']),
    backgroundPosition: BACKGROUND_SIZE_VALIDATOR
  },
  text: {
  ...
  transform: {
    ...
    transformOrigin: SHORTHAND_LENGTH_VALIDATOR
  },
  animation: {
    animationName: DEFAULT_VALIDATOR,
    animationDelay: TRANSITION_INTERVAL_VALIDATOR,
    animationDuration: TRANSITION_INTERVAL_VALIDATOR,
    animationDirection: genEnumValidator(['normal', 'reverse', 'alternate', 'alternate-reverse']),
    animationIterationCount: ANIMATION_ITERATION_VALIDATOR,
    animationTimingFunction: TRANSITION_TIMING_FUNCTION_VALIDATOR,
    animationFillMode: genEnumValidator(['forwards', 'backwards', 'both']),
  },
  customized: {
